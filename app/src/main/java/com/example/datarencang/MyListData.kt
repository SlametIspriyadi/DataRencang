...
import com.example.datarencang.databinding.ActivityMyListDataBinding
...
class MyListData : AppCompatActivity(), RecyclerViewAdapter.RecyclerViewAdaptor.dataListener {
    ...
    private var dataTeman = ArrayList<data_rencang>()
    ...
    private fun GetData(){
        ...
        getReference.child("Admin").child(getUserID).child("DataRencang")
                ...
                override fun onDataChange(dataSnapshot: DataSnapshot) {
                    if (dataSnapshot.exists()){
                        ...
                        for (snapshot in dataSnapshot.children){
                            ...
                            val teman = snapshot.getValue(data_rencang::class.java)
                        ...
                        adapter =
                            RecyclerViewAdapter.RecyclerViewAdaptor(dataTeman, this@MyListData)
                        //memasang adapter pd RecyclerView
                        recyclerView?.adapter = adapter
                        (adapter as RecyclerViewAdapter.RecyclerViewAdaptor).notifyDataSetChanged()
    ...
    override fun onDeleteData(data: data_rencang?, position: Int){
        val getUserID: String = auth?.getCurrentUser()?.getUid().toString()
        val getReference = database.getReference()
        if (getReference != null) {
            getReference.child("Admin").child(getUserID).child("DataRencang").child(data?.key.toString())
                .removeValue()
